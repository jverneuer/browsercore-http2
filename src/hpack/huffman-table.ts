/**
 * HPACK Huffman coding table (RFC 7541 Appendix B).
 *
 * Each row is `[bits, code, symbol]` where `code` is the Huffman code left-aligned
 * into a 32-bit word with `bits` valid bits — matching the RFC's "MSB-aligned"
 * representation. The table is sorted by (bits, code) ascending so the decoder
 * walks codes short-to-long and matches greedily.
 */

/** A single Huffman decode-table row: `code` occupies its lowest `bits`. */
...
/**
 * The full HPACK Huffman table (Appendix B). To decode, read `bits` at a time;
 * the table below is ordered so the decoder matches the longest valid prefix.
 */
